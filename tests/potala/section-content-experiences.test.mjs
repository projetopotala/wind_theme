...
test("a busca da Biblioteca encontra título, tema e formato sem diferenciar acentos", async () => {
  const modulo = await import("../../outputs/js/culture/library-catalog.js").catch(() => null);
  assert.ok(modulo, "falta a experiência local da Biblioteca Potala");
  ...
  assert.deepEqual(modulo.filterLibraryCatalog(catalog, "saude"), [catalog[0]]);
  assert.deepEqual(modulo.filterLibraryCatalog(catalog, "digital"), [catalog[1]]);
  assert.deepEqual(modulo.filterLibraryCatalog(catalog, "   "), catalog);
});

test("a Revista filtra pautas por tema e por busca textual", async () => {
  const modulo = await import("../../outputs/js/revista/magazine-filter.js").catch(() => null);
  assert.ok(modulo, "falta o filtro editorial da Revista");
  ...
  assert.deepEqual(modulo.filterMagazineArticles(articles, { category: "saude", query: "" }), [articles[0]]);
  assert.deepEqual(modulo.filterMagazineArticles(articles, { category: "todos", query: "musica" }), [articles[1]]);
  assert.deepEqual(modulo.filterMagazineArticles(articles, { category: "todos", query: "" }), articles);
});
...
test("o front acrescenta a Revista ausente sem apagar a edição remota", async () => {
  const modulo = await import("../../outputs/js/home/default-section-bridge.js").catch(() => null);
  assert.ok(modulo, "falta a ponte local da nova seção");
  ...
  const missing = modulo.mergeRequiredDefaultSections(
  ...
  const edited = modulo.mergeRequiredDefaultSections(
...
test("cada encontro cultural abre conteúdo próprio na mesma página editorial", async () => {
  const modulo = await import("../../outputs/js/culture/cultural-experiences.js").catch(() => null);
  assert.ok(modulo, "falta o catálogo das experiências culturais");
  ...
  for (const [slug, title] of Object.entries(expected)) {
    const experience = modulo.culturalExperienceFor(slug);
...
test("a respiração 3-3-3 percorre inspirar, segurar e expirar", async () => {
  const modulo = await import("../../outputs/js/inspiration/practice-studio.js").catch(() => null);
  assert.ok(modulo, "falta o estúdio funcional de inspiração");

  assert.deepEqual(modulo.breathingStateAt(0), { phase: "Inspirar", phaseSecond: 0, cycleSecond: 0 });
  assert.deepEqual(modulo.breathingStateAt(3), { phase: "Segurar", phaseSecond: 0, cycleSecond: 3 });
  assert.deepEqual(modulo.breathingStateAt(6), { phase: "Expirar", phaseSecond: 0, cycleSecond: 6 });
  assert.deepEqual(modulo.breathingStateAt(9), { phase: "Inspirar", phaseSecond: 0, cycleSecond: 0 });
});
...
test("meditação e escuta terminam e a reflexão pronta permanece disponível", async () => {
  ...
  assert.equal(modulo.timedPracticeState("meditar", 0).remaining, 60);
  assert.equal(modulo.timedPracticeState("meditar", 60).complete, true);
  assert.equal(modulo.timedPracticeState("escutar", 30).complete, true);
  assert.match(modulo.reflectionAt(0), /\?/);
});

test("uma prática pode ser pausada e continuada sem perder o ponto", async () => {
  const modulo = await import("../../outputs/js/inspiration/practice-studio.js");
  let state = modulo.transitionPracticeControl(undefined, "start", 1_000);

  assert.equal(state.status, "running");
  state = modulo.transitionPracticeControl(state, "pause", 5_500);
  assert.equal(state.status, "paused");
  assert.equal(modulo.practiceElapsedSeconds(state, 9_000), 4);

  state = modulo.transitionPracticeControl(state, "resume", 9_000);
  assert.equal(state.status, "running");
  assert.equal(modulo.practiceElapsedSeconds(state, 11_500), 7);

  state = modulo.transitionPracticeControl(state, "reset", 12_000);
...
test("as práticas complementares de Inspiração abrem orientações utilizáveis", async () => {
  ...
  for (const resource of ["meditacao", "mantra", "receita", "gentileza", "bem-estar"]) {
    ...
    const content = modulo.inspirationResourceFor(resource);
