...
export function JourneyOverlay({ checkpoint, reducedMotion = false }: { checkpoint: JourneyCheckpointType; reducedMotion?: boolean }) {
  const content = JOURNEY_CONTENT[checkpoint.contentId];
  if (!content) return null;
  if (content.kind === "epilogue") return null;
  return (
    <div className={styles.overlay} aria-live="polite" aria-atomic="true">
      <div className={styles.overlayInner} key={content.id}>
        {content.kind === "arrival"
          ...
          : <JourneyCheckpoint content={content} active reducedMotion={reducedMotion} />}
      </div>
    </div>
  );
}
